...
proc purgeHandler*(b: TgManager, c: Command) {.async.} =
    let response = c.message
    var msgTxt: string
    if not (await canBotDelete(b, response)):
        discard await b.bot.sendMessage(response.chat.id, "I can't delete messages!", replyToMessageId = response.messageId)
        return

    if not (await isUserAdm(b, response.chat.id.int, response.fromUser.get.id)):
        discard await b.bot.sendMessage(response.chat.id, "You aren't Adm :^(", replyToMessageId = response.messageId)
        return

    if response.replyToMessage.isSome:
        for msgid in countdown(response.messageId, response.replyToMessage.get.messageId):
            try:
                discard await deleteMessage(b.bot, $response.chat.id.int, msgid)
            except IOError:
                continue
...
proc delHandler*(b: TgManager, c: Command) {.async.} =
    let response = c.message
    var msgTxt: string
    if await canBotDelete(b, response):
        if await isUserAdm(b, response.chat.id.int, response.fromUser.get.id):
            if response.replyToMessage.isSome:
                discard await deleteMessage(b.bot, $response.chat.id.int, response.replyToMessage.get.messageId)
